...
class Options_Framework_Media_Uploader {
    ...
    static function optionsframework_uploader( $_id, $_value, $_desc = '', $_name = '' ) {
        ...
        $output .= '<input id="' . $id . '" class="upload' . $class . '" type="text" name="'.$name.'" value="' . $value . '" placeholder="' . __('未选择任何文件', 'mlldxe') .'" />' . "\n";
        if ( function_exists( 'wp_enqueue_media' ) ) {
            if ( ( $value == '' ) ) {
                $output .= '<input id="upload-' . $id . '" class="upload-button button" type="button" value="' . __( '上传', 'mlldxe' ) . '" />' . "\n";
            } else {
                $output .= '<input id="remove-' . $id . '" class="remove-file button" type="button" value="' . __( '移除', 'mlldxe' ) . '" />' . "\n";
            }
        } else {
            $output .= '<p><i>' . __( '请更新您的 WordPress 版本以获取完整的媒体文件支持。', 'mlldxe' ) . '</i></p>';
        }
        if ( $_desc != '' ) {
